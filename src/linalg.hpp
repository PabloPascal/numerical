...
public:
    ...
    vec(std::vector<T>&& data) : Tensor<T>(data.size(), 1, std::move(data)) {}
    vec(std::vector<T>& data) : Tensor<T>(data.size(), 1, data) {}


    T length(){
        T sq_sum = (T)0;
        for(size_t i = 0; i < this->_rows; ++i){
            sq_sum += std::pow(this->_data[i], 2);
        }
...
T dot_product(const vec<T>& a, const vec<T>& b){
   ...
   if(a.size() != b.size()) throw std::invalid_argument("not same size");
...
template <std::floating_point T>
Tensor<T> hadamard_product(const Tensor<T>& a, const Tensor<T>& b){
...
template <std::floating_point T>
Tensor<T> operator/(const Tensor<T>& tensor, T alpha){
